...
// ── Cambiar video de fondo y color de acento segun condicion ─
function updateBackground(condition) {
  const map = VIDEO_MAP[condition] || VIDEO_MAP['Clouds'];
  const video = document.getElementById('bgVideo');

  // Color de acento (temperatura, etc.) siempre se actualiza,
  // aunque el video ya sea el mismo
  ...
  video.load();
  video.play().catch(() => {
    // Autoplay bloqueado — el <video poster> ya muestra la imagen de fallback
    console.warn('Autoplay bloqueado, se muestra el poster:', map.poster);
  });
